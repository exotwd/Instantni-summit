...
async function checkIn(id) { const d = state.delegations.find(x => x.id === id); const name = prompt("Jméno účastníka", d.participant?.name || "") || ""; const email = prompt("E-mail účastníka", d.participant?.email || "") || ""; const note = prompt("Poznámka", d.participant?.note || "") || ""; await post("/api/attendance/check-in", { delegationId: id, note, participant: { delegationId: id, name, email, note } }); }
