...
func (r *DelegationRepository) List(ctx context.Context, includeParticipants bool) ([]domain.Delegation, error) {
	rows, err := r.db.QueryContext(ctx, `select d.id,d.name,d.code,d.flag,d.access_code,d.access_code_created_at,d.present,d.display_order,d.created_at,d.updated_at,
...
func (r *DelegationRepository) Get(ctx context.Context, id int64, includeParticipant bool) (*domain.Delegation, error) {
	row := r.db.QueryRowContext(ctx, `select d.id,d.name,d.code,d.flag,d.access_code,d.access_code_created_at,d.present,d.display_order,d.created_at,d.updated_at,
		s.id,s.x,s.y,s.w,s.h,s.rotation,s.revision,s.updated_at,
		p.id,p.name,p.email,p.co_delegate_name,p.co_delegate_email,p.note,p.created_at,p.updated_at
		from delegations d
		left join seat_layout s on s.delegation_id = d.id
...
func (r *DelegationRepository) FindByAccessCode(ctx context.Context, code string) (*domain.Delegation, error) {
	row := r.db.QueryRowContext(ctx, `select d.id,d.name,d.code,d.flag,d.access_code,d.access_code_created_at,d.present,d.display_order,d.created_at,d.updated_at,
		...
		left join seat_layout s on s.delegation_id = d.id
		left join participants p on p.delegation_id = d.id
		where d.access_code = ? and d.present = true`, code)
...
func (r *DelegationRepository) UpdateBasic(ctx context.Context, d domain.Delegation) error {
	_, err := r.db.ExecContext(ctx, `update delegations set name=?, code=?, flag=?, display_order=?, updated_at=current_timestamp where id=?`,
		d.Name, d.Code, d.Flag, d.DisplayOrder, d.ID)
	return err
}

func (r *DelegationRepository) SetPresenceAndCode(ctx context.Context, id int64, present bool, code string) error {
	if present {
		_, err := r.db.ExecContext(ctx, `update delegations set present=true, access_code=?, access_code_created_at=current_timestamp, updated_at=current_timestamp where id=?`, code, id)
		return err
	}
	_, err := r.db.ExecContext(ctx, `update delegations set present=false, access_code=null, access_code_created_at=null, updated_at=current_timestamp where id=?`, id)
	return err
}

func (r *DelegationRepository) AccessCodeExists(ctx context.Context, code string) (bool, error) {
	var count int
	err := r.db.QueryRowContext(ctx, `select count(*) from delegations where access_code = ? and present = true`, code).Scan(&count)
	return count > 0, err
}
...
func scanDelegation(row delegationScanner, includeParticipant bool) (domain.Delegation, error) {
	...
	err := row.Scan(&d.ID, &d.Name, &d.Code, &d.Flag, &access, &accessCreated, &d.Present, &d.DisplayOrder, &d.CreatedAt, &d.UpdatedAt,
